...
const createProject = z.object({
    ...
    keywords: z
        .array(z.string())
        .optional()
        .describe("Array of tracked keywords for this project. Can enter tags separated by commas after each keyword"),
    tags: z
        .array(z.object({
        tag: z.string(),
        ...
        .optional()
        .describe("Optional array of tags for keywords. Structure is {tag: String, keywords: String[]}."),
    competitors: z
        .array(z.string())
        .optional()
...
const getProjectKeywords = z.object({
    ...
    tags: z
        .array(z.string())
        .optional()
        .describe("Keep only keywords with specified tags. Tags must be defined within the project."),
    lineCount: z
        .number()
        .default(20)
...
export function configureHaloscanServer(server) {
    ...
    /* -------------------- KEYWORDS TOP SITES -------------------- */
    server.tool("get_keywords_top_sites", "Obtenir les sites les plus pertinents pour une liste de mots-clés.", getKeywordsTopSites.shape, async (params, extra) => {
        const apiKey = getApiKeyFromContext(extra);
        if (!apiKey) {
            return {
                isError: true,
        ...
        try {
            const data = await makeHaloscanRequest("/keywords/topSites", params, "POST", apiKey);
            return {
    ...
    server.tool("get_keywords_serp_history", "Obtenir l'historique SERP d'un mot-clé.", getKeywordsSerpHistory.shape, async (params, extra) => {
        const apiKey = getApiKeyFromContext(extra);
        if (!apiKey) {
            return {
                isError: true,
        ...
        try {
            const data = await makeHaloscanRequest("/keywords/serpHistory", params, "POST", apiKey);
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
            };
        }
    ...
    server.tool("get_keywords_serp_domain_evolution", "Obtenir l'évolution historique d'un domaine dans les SERP d'un mot-clé.", getKeywordsSerpDomainEvolution.shape, async (params, extra) => {
        const apiKey = getApiKeyFromContext(extra);
        if (!apiKey) {
            return {
                isError: true,
        ...
        try {
            const data = await makeHaloscanRequest("/keywords/serp/domainEvolution", params, "POST", apiKey);
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
            };
        }
    ...
    server.tool("get_domains_aio_sources", "Obtenir les sources AIO d'un domaine ou d'une URL", getDomainsAioSources.shape, async (params, extra) => {
        const apiKey = getApiKeyFromContext(extra);
        if (!apiKey) {
            return {
                isError: true,
        ...
        try {
            const data = await makeHaloscanRequest("/domains/aioSources", params, "POST", apiKey);
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
            };
        }
    ...
    /* -------------------- CREATE PROJECT -------------------- */
    server.tool("create_project", "Créer un projet.", createProject.shape, async (params, extra) => {
        const apiKey = getApiKeyFromContext(extra);
        if (!apiKey) {
            return {
                isError: true,
        ...
        try {
            const data = await makeHaloscanRequest("/projects/create", params, "POST", apiKey);
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
            };
        }
    ...
    /* -------------------- UPDATE PROJECT -------------------- */
    server.tool("update_project", "Mettre à jour un projet avec son identifiant.", updateProject.shape, async (params, extra) => {
        const apiKey = getApiKeyFromContext(extra);
        if (!apiKey) {
            return {
                isError: true,
        ...
        try {
            ...
            const data = await makeHaloscanRequest(`/projects/${project_id}`, body, "POST", apiKey);
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
            };
        }
    ...
    /* -------------------- DELETE PROJECT -------------------- */
    server.tool("delete_project", "Supprimer définitivement un projet avec son identifiant.", deleteProject.shape, async (params, extra) => {
        const apiKey = getApiKeyFromContext(extra);
        if (!apiKey) {
            return {
                isError: true,
        ...
        try {
            const data = await makeHaloscanRequest(`/projects/${params.project_id}`, {}, "DELETE", apiKey);
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
            };
        }
    ...
    /* -------------------- LIST PROJECTS -------------------- */
    server.tool("list_projects", "Obtenir la liste des projets existants avec leurs mots-clés et leurs tags.", listProjects.shape, async (_params, extra) => {
        const apiKey = getApiKeyFromContext(extra);
        if (!apiKey) {
            return {
                isError: true,
        ...
        try {
            const data = await makeHaloscanRequest("/projects/list", {}, "GET", apiKey);
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
            };
        }
    ...
    /* -------------------- PROJECT DETAILS -------------------- */
    server.tool("get_project_details", "Obtenir tous les paramètres d'un projet avec son identifiant.", getProjectDetails.shape, async (params, extra) => {
        const apiKey = getApiKeyFromContext(extra);
        if (!apiKey) {
            return {
                isError: true,
        ...
        try {
            const data = await makeHaloscanRequest(`/projects/${params.project_id}/details`, {}, "POST", apiKey);
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
            };
        }
    ...
    /* -------------------- PROJECTS OVERVIEW -------------------- */
    server.tool("get_projects_overview", "Obtenir un aperçu des projets, des positions, de la visibilité et des statistiques des mots-clés.", getProjectsOverview.shape, async (params, extra) => {
        const apiKey = getApiKeyFromContext(extra);
        if (!apiKey) {
            return {
                isError: true,
        ...
        try {
            const data = await makeHaloscanRequest("/projects/overview", params, "POST", apiKey);
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
            };
        }
    ...
    /* -------------------- PROJECT OVERVIEW -------------------- */
    server.tool("get_project_overview", "  Obtenir les données d'aperçu et les graphiques d'un projet spécifique.", getProjectOverview.shape, async (params, extra) => {
        const apiKey = getApiKeyFromContext(extra);
        if (!apiKey) {
            return {
                isError: true,
        ...
        try {
            const { project_id, ...body } = params;
            const data = await makeHaloscanRequest(`/projects/${project_id}/overview`, body, "POST", apiKey);
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
            };
        }
    ...
    /* -------------------- PROJECT KEYWORDS -------------------- */
    server.tool("get_project_keywords", "Obtenir les données de suivi des positions des mots-clés d'un projet.", getProjectKeywords.shape, async (params, extra) => {
        const apiKey = getApiKeyFromContext(extra);
        if (!apiKey) {
            return {
                isError: true,
        ...
        try {
            const { project_id, ...body } = params;
            const data = await makeHaloscanRequest(`/projects/${project_id}/keywords`, body, "POST", apiKey);
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
            };
        }
    ...
    /* -------------------- PROJECT TRACKING -------------------- */
    server.tool("get_project_tracking", "Obtenir les données graphiques de suivi des positions des mots-clés d'un projet.", getProjectTracking.shape, async (params, extra) => {
        const apiKey = getApiKeyFromContext(extra);
        if (!apiKey) {
            return {
                isError: true,
        ...
        try {
            const { project_id, ...body } = params;
            const data = await makeHaloscanRequest(`/projects/${project_id}/tracking`, body, "POST", apiKey);
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
            };
        }
