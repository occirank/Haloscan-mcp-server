...
export function configureHaloscan(server) {
    ...
    /* -------------------- KEYWORDS TOP SITES -------------------- */
    server.tool("get_keywords_top_sites", "Obtenir les sites les plus pertinents pour une liste de mots-clés.", getKeywordsTopSites.shape, async (params) => {
        const apiKey = API_KEY;
        if (!apiKey) {
            return {
                isError: true,
    ...
    server.tool("get_keywords_serp_history", "Obtenir l'historique SERP d'un mot-clé.", getKeywordsSerpHistory.shape, async (params) => {
        const apiKey = API_KEY;
        if (!apiKey) {
            return {
                isError: true,
    ...
    server.tool("get_keywords_serp_domain_evolution", "Obtenir l'évolution historique d'un domaine dans les SERP d'un mot-clé.", getKeywordsSerpDomainEvolution.shape, async (params) => {
        const apiKey = API_KEY;
        if (!apiKey) {
            return {
                isError: true,
    ...
    server.tool("get_domains_aio_sources", "Obtenir les sources AIO d'un domaine ou d'une URL", getDomainsAioSources.shape, async (params) => {
        const apiKey = API_KEY;
        if (!apiKey) {
            return {
                isError: true,
    ...
    /* -------------------- CREATE PROJECT -------------------- */
    server.tool("create_project", "Créer un projet.", createProject.shape, async (params) => {
        const apiKey = API_KEY;
        if (!apiKey) {
            return {
                isError: true,
    ...
    /* -------------------- UPDATE PROJECT -------------------- */
    server.tool("update_project", "Mettre à jour un projet avec son identifiant.", updateProject.shape, async (params) => {
        const apiKey = API_KEY;
        if (!apiKey) {
            return {
                isError: true,
    ...
    /* -------------------- DELETE PROJECT -------------------- */
    server.tool("delete_project", "Supprimer définitivement un projet avec son identifiant.", deleteProject.shape, async (params) => {
        const apiKey = API_KEY;
        if (!apiKey) {
            return {
                isError: true,
    ...
    /* -------------------- LIST PROJECTS -------------------- */
    server.tool("list_projects", "Obtenir la liste des projets existants avec leurs mots-clés et leurs tags.", listProjects.shape, async (_params) => {
        const apiKey = API_KEY;
        if (!apiKey) {
            return {
                isError: true,
    ...
    /* -------------------- PROJECT DETAILS -------------------- */
    server.tool("get_project_details", "Obtenir tous les paramètres d'un projet avec son identifiant.", getProjectDetails.shape, async (params) => {
        const apiKey = API_KEY;
        if (!apiKey) {
            return {
                isError: true,
    ...
    /* -------------------- PROJECTS OVERVIEW -------------------- */
    server.tool("get_projects_overview", "Obtenir un aperçu des projets, des positions, de la visibilité et des statistiques des mots-clés.", getProjectsOverview.shape, async (params) => {
        const apiKey = API_KEY;
        if (!apiKey) {
            return {
                isError: true,
    ...
    /* -------------------- PROJECT OVERVIEW -------------------- */
    server.tool("get_project_overview", "  Obtenir les données d'aperçu et les graphiques d'un projet spécifique.", getProjectOverview.shape, async (params) => {
        const apiKey = API_KEY;
        if (!apiKey) {
            return {
                isError: true,
    ...
    /* -------------------- PROJECT KEYWORDS -------------------- */
    server.tool("get_project_keywords", "Obtenir les données de suivi des positions des mots-clés d'un projet.", getProjectKeywords.shape, async (params) => {
        const apiKey = API_KEY;
        if (!apiKey) {
            return {
                isError: true,
    ...
    /* -------------------- PROJECT TRACKING -------------------- */
    server.tool("get_project_tracking", "Obtenir les données graphiques de suivi des positions des mots-clés d'un projet.", getProjectTracking.shape, async (params) => {
        const apiKey = API_KEY;
        if (!apiKey) {
            return {
                isError: true,
